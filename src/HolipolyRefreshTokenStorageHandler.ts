...
export const getStorageAuthEventKey = (prefix?: string) =>
  [prefix, "holipoly_storage_auth_change"].filter(Boolean).join("+");
export const getStorageAuthStateKey = (prefix?: string) =>
  [prefix, "holipoly_auth_module_auth_state"].filter(Boolean).join("+");
export const getRefreshTokenKey = (prefix?: string) =>
  [prefix, "holipoly_auth_module_refresh_token"].filter(Boolean).join("+");

export type AuthState = "signedIn" | "signedOut";

export type HolipolyAuthEvent = CustomEvent<{ authState: AuthState }>;

export class HolipolyRefreshTokenStorageHandler {
  constructor(
