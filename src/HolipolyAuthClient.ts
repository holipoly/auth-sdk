import { HolipolyRefreshTokenStorageHandler } from "./HolipolyRefreshTokenStorageHandler";
...
import { HolipolyAccessTokenStorageHandler } from "./HolipolyAccessTokenStorageHandler";

export interface HolipolyAuthClientProps {
...
export class HolipolyAuthClient {
  ...
  private holipolyApiUrl: string;
  /**
   * Persistent storage (for refresh token)
   */
  private refreshTokenStorage: HolipolyRefreshTokenStorageHandler | null;

  /**
   * Non-persistent storage for access token
   */
  private acessTokenStorage: HolipolyAccessTokenStorageHandler;
  ...
  fetchWithAuth: FetchWithAdditionalParams = async (input, init, additionalParams) => {
    ...
    if (!this.acessTokenStorage.getAccessToken() && typeof document !== "undefined") {
      // this flow is used by HolipolyExternalAuth
...
export const createHolipolyAuthClient = (props: HolipolyAuthClientProps) => new HolipolyAuthClient(props);
