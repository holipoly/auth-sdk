...
import { HolipolyAuthClient } from "../src/HolipolyAuthClient";
import { getRefreshTokenKey } from "../src/HolipolyRefreshTokenStorageHandler";
import type { StorageRepository } from "../src";

describe("HolipolyAuthClient", () => {
  const mockStorage = {
    getItem: vi.fn(),
    setItem: vi.fn(),
    removeItem: vi.fn(),
  } satisfies StorageRepository;
  const masterStagingUrl = "https://master.staging.holipoly.cloud/graphql/";
  const otherApiUrl = "https://some-other-domain-auth-sdk.holipoly.cloud/graphql/";

  it(`should fetch without authentication token`, async () => {
    const onAuthRefresh = vi.fn();
    const holipolyAuthClient = new HolipolyAuthClient({
      holipolyApiUrl: masterStagingUrl,
      refreshTokenStorage: mockStorage,
      onAuthRefresh,
    });

    await holipolyAuthClient.fetchWithAuth(masterStagingUrl, {
      method: "POST",
      body: "{}",
    });
  ...
  it(`should not add auth token to external URLs`, async () => {
    ...
    await holipolyAuthClient.fetchWithAuth(otherApiUrl, {
      method: "POST",
      body: "{}",
    });
  ...
  it(`should add auth token to external URLs when allowPassingTokenToThirdPartyDomains=true`, async () => {
    ...
    await holipolyAuthClient.fetchWithAuth(
      otherApiUrl,
      {
        method: "POST",
        body: "{}",
